...
export const login = async (req, res) => {
  ...
  res.cookie("token", token, {
    httpOnly: false, // Allow cookie to be accessed via JavaScript
    secure: false, // Set to true if using HTTPS
    path: '/', // Set to '/' to make cookie accessible across all paths
  });

  return res.status(200).json({ name: user.nombre, _id: user._id , token: token});
};
